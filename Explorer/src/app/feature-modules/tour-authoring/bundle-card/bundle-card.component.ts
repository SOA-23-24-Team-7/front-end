...
export class BundleCardComponent {
  ...
  publish(): void {
    if (this.bundle.bundleItems.length < 2) {
      alert("You must have at least 2 published tours to publish a bundle.");
      return;
    }
  ...
  addBundleOrderItem(): void {
    ...
    let bundleIds = this.shoppingCart.bundleOrderItems?.map(boi => boi.bundleId);
  ...
  removeBundle(): void {
    this.shoppingCart.bundleOrderItems?.forEach(boi => {
      if (boi.bundleId == this.bundle.id) {
        this.marketplaceService.removeBundleOrderItem(boi.id!).subscribe({
          next: (result: any) => {
